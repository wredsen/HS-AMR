...
import parkingRobot.INavigation3.ParkingSlot.ParkingSlotStatus;
import parkingRobot.hsamr1.NavigationThread;
import parkingRobot.hsamr1.GuiDemo;
 ...
 * @author IfA
 */
public class NavigationAT3 implements INavigation3{
	
	INavigation3.ParkingSlot[] Slots=null;  
	...
	*/
	 ...
	 */
	public NavigationAT3(IPerception perception, IMonitor monitor){
	...
	private void calculateLocation(){
		...
		if(CurrentStatus.LINE_FOLLOW==GuiDemo.currentStatus || CurrentStatus.LINE_FOLLOW_2==GuiDemo.currentStatus){
	...
	*/ 
	...
	private void evaluateCornerDetection() {
		switch(lastCornerNumber){
			case 0: 
				if (GuiDemo.getAnfahrt() == false)	this.pose.setLocation((float)0.00,(float)0.00);
				nextCornerNumber = 1;
				break; 
			case 1: 
				if (GuiDemo.getAnfahrt() == false)	this.pose.setLocation((float)1.80,(float)0.00);
				nextCornerNumber = 2;
			...
			case 3: 
				...
				break;
			case 4:
				if (GuiDemo.getAnfahrt() == false)	this.pose.setLocation((float)1.50,(float)0.30);
				nextCornerNumber = 5;
				break;
			case 5:
	...
	*/
	...
	private boolean checkDublicate() {
		...
		if (slotList.size()>0) {
			for(int i =0; i<slotList.size() ;i++) {
				...
				INavigation3.ParkingSlot slot = slotList.get(i);
	...
	private void addParkingSlot() {
		if(this.newBackBoundaryPosition.getY() == this.newFrontBoundaryPosition.getY()) {	/*do nothing*/	}
			...
			INavigation3.ParkingSlot slot = new ParkingSlot(ID,newFrontBoundaryPosition,newBackBoundaryPosition,status,slotMeasurementQuality);
			slotList.add(slot);
	...
	private void overwriteParkingSlot () {
		if(this.newBackBoundaryPosition.getY() == this.newFrontBoundaryPosition.getY()) {	/*do nothing*/	}
			...
			INavigation3.ParkingSlot slot = new ParkingSlot((slotListIndex+1),newFrontBoundaryPosition,newBackBoundaryPosition,status,slotMeasurementQuality);
