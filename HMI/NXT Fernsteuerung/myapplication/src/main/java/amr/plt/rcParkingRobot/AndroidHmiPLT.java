...
public class AndroidHmiPLT implements IAndroidHmi { 
	...
	public List<ParkingSlot> getArray(){
		return parkSlotHandler.parkingSlots;
	}
	...
	static class ParkSlotHandler extends Handler {
		...
		@Override
		public void handleMessage(Message msg) {
			ParkingSlot pslot = (ParkingSlot) msg.obj;
			if(pslot.ID <= parkingSlots.size()) {
				parkingSlots.remove(pslot.ID - 1);
			}
			parkingSlots.add(pslot.ID -1, pslot);
