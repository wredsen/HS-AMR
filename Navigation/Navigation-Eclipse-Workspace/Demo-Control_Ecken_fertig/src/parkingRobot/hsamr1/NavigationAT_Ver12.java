...
public class NavigationAT_Ver12 implements INavigation{
	...
	public boolean detectCorner() {
		...
		if (this.angleList.size()>=amountOfValues) {
			...
			if (Math.abs(newFirstValue - oldFirstValue) >= 0.25*Math.PI) {
				if (Math.abs(newSecondValue - oldSecondValue) >=0.25*Math.PI) {
					if (Math.abs(newSecondValue-newFirstValue) >= 0.33*Math.PI) {
