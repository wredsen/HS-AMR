...
public class NavigationAT_Ver1 implements INavigation{
	...
	public synchronized void updateNavigation(){	
		...
		LCD.drawString("X = "+ this.pose.getX(), 0, 5);
		LCD.drawString("Y = "+ this.pose.getY(), 0, 6);
	...
	public synchronized boolean getCornerArea() {
		...
		if (mapModus == 1) {
			...
			if ((this.pose.getX()>=1.70)&&(this.pose.getY()<=0.07)) {
				safety = true;
			}
	...
	public boolean detectCorner() {
		...
		if (rightLightSensorList.size()>=2) {
			if (((rightLightSensorList.get(1)-rightLightSensorList.get(0)>=35)||(leftLightSensorList.get(1)-leftLightSensorList.get(0)>=35))) {
				corner = true;
				//Sound.twoBeeps();
				cornerNumber++;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////			
	...
	private void detectParkingSlot(){
		...
		if(slotList.size() > 0) {
		  //1. In ArrazList Parkplatz schon da��
		  //Wenn Ja: Abbruch
		  //Wenn Nein:Parkplatz hinzufuegen
