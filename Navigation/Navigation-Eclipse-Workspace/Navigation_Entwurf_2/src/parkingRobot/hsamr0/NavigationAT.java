...
public class NavigationAT implements INavigation{
	...
	float testvariable2 = 0;
	float testvariable3 = 0;
	
	/**
	 *  Holds the length of the Arrays, can only be a even number;
	 */
	 ...
	 */
	int[] rightLightSensorArray = new int[arrayLength];
	
	/**
	 * holds the last 10 values from the left Light Sensor
	 */
	int[] leftLightSensorArray = new int[arrayLength];
	...
	public synchronized void updateNavigation(){	
		...
		i = i%arrayLength;													
		rightLightSensorArray[i] = lineSensorRight;					// Sampling of the lightsensor values for corner detection
		leftLightSensorArray[i] = lineSensorLeft;
		i ++;
		
		cornerNumber = detectCorner();
		
		float differenceLeftLightsensorArray = calculateAverageDifference(leftLightSensorArray);
		testvariable1 = differenceLeftLightsensorArray;
		
		float differenceRightLightsensorArray = calculateAverageDifference(rightLightSensorArray);
		testvariable2 = differenceRightLightsensorArray;
		
		
		LCD.clear();	
		LCD.drawString("Diff. L =" + testvariable1, 0, 0);
		LCD.drawString("Diff. R =" + testvariable2, 0, 1);
		LCD.drawString("Nummer =" + cornerNumber, 0, 3);
	...
	/**
	 * Calculates the difference between the average values of the first and the second half of an array
	 * @param array
	 * @return d
	 */
	private float calculateAverageDifference (int[] array) {
		float  d = 0;  
		int sum1 = 0;
		int sum2 = 0;
		float a1 = 0;
	...
	private int detectCorner() {
		...
		if ((differenceLeftLightsensorArray<50) && (differenceRightLightsensorArray>50)) {
			leftCorner = true;
			cornerNumber++;
		}
		
		if ((differenceRightLightsensorArray<50) && (differenceLeftLightsensorArray>50)) {
			rightCorner = true;
			cornerNumber++;
		}
