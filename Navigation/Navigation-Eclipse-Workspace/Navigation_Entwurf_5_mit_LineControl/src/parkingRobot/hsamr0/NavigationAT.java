...
public class NavigationAT implements INavigation{
	...
	public boolean detectCorner() {
		...
		rightCorner = false;
		leftCorner = false;
		if(( getAverageDiff(rightLightSensorList) < lowLimit ) && ( getAverageDiff(leftLightSensorList) < highLimit )) {
			corner = true;
			leftCorner=true;
			cornerNumber++;
		...
		if (( getAverageDiff(leftLightSensorList) < lowLimit ) && ( getAverageDiff(rightLightSensorList) < highLimit )) {
			corner = true;
			rightCorner = true;
			cornerNumber++;
			cornerNumber = cornerNumber % 8;		
