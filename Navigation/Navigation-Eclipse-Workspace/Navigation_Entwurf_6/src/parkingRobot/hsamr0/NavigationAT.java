...
public class NavigationAT implements INavigation{
	...
	public boolean detectCorner() {
		...
		boolean safety = true;					// ANPASSEN!!
		rightCorner = false;
		leftCorner = false;
		
		
		/*
		if ((this.pose.getX()>=0.5)&&(this.pose.getY()<=0.1)) {
			safety = true;
		}
	
		if ((this.pose.getX()>=170)&&(this.pose.getY()<=10)) {
			safety = true;
		}
		
		if ((this.pose.getX()>=170)&&(this.pose.getY()>=50)) {
			safety = true;
		}
		
		if ((this.pose.getX()>=140)&&(this.pose.getY()>=50)) {
			safety = true;
		}
		...
		*/
			...
			if (rightLightSensorList.size()>=2) {
				if (((rightLightSensorList.get(1)-rightLightSensorList.get(0)>=70)||(leftLightSensorList.get(1)-leftLightSensorList.get(0)>=70)) && safety== true) {
					corner = true;
					Sound.twoBeeps();
					cornerNumber++;
				}	
