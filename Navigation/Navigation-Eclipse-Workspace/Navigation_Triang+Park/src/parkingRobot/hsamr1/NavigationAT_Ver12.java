		...
		if(slotList.size() > 0) {
			...
			{	
				for(int i =0; i<slotList.size();i++) {
					...
					if(((yBackAktuell - 0.1) <= yBackListe) && (yBackListe <= (yBackAktuell + 0.1))){
						parkplatzVorhanden = true;
					}
					...
					if(((yFrontAktuell - 0.1) <= yFrontListe) && (yFrontListe <= (yFrontAktuell + 0.1))){
						parkplatzVorhanden = true;
					}
			...
			for(int i =0; i<slotList.size();i++) {
				...
				if(((xBackAktuell - 0.1) <= xBackListe) && (xBackListe <= (xBackAktuell + 0.1))){
					parkplatzVorhanden = true;
				}
				...
				if(((xFrontAktuell - 0.1) <= xFrontListe) && (xFrontListe <= (xFrontAktuell + 0.1))){
					parkplatzVorhanden = true;
				}
