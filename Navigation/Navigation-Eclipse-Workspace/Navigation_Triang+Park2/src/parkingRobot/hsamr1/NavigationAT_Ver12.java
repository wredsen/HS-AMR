		...
		if (slotList.size()>0) {
			...
			for(int i =0; i<slotList.size() ;i++) {
				...
				if ((x_new_Back > 1.70)) {										// check y-values if the detected slot is vertical
					if (Math.abs(y_new_Back - y_Back)< 0.10) return true;			
					if (Math.abs(y_new_Front - y_Front)< 0.10) return true;
				}
			}
		}
		else return false;
	...
	*/
	private boolean checkParkingSlotArea() {
		if (mapModus == 1) {
			...
			if (this.pose.getX() > 1.75 && (Math.abs(this.pose.getHeading()-0.5*Math.PI)<=0.2*Math.PI) ) {
				verticalSlot = true;
				return true;
			}
		
			if ((this.pose.getX()>=0.35) && (this.pose.getX()<=1.45) && (this.pose.getHeading()<=1.2*Math.PI)) {
				verticalSlot = false;
				return true;
			}
			else {
	...
	private void detectParkingSlot(){
		if (this.checkParkingSlotArea() == true) {
			...
			if ( (foundBackBoundary == true) && (detectFrontBoundary() == true)) {
				...
				if (checkDublicate() == false) this.saveParkingSlot();
			}
		}
		return;
	}
